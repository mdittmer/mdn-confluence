...
foam.CLASS({
  ...
  requires: ['org.mozilla.mdn.CompatJson', 'org.mozilla.mdn.CompatJsonAdapter'],
  ...
  properties: [
    ...
    },
    {
      name: 'mdnApis_',
      factory: function () {
        return this.bcd_.api;
      },
    },
    {
      name: 'mdnBrowsers_',
      factory: function () {
        return this.bcd_.browsers;
      },
    },
    {
      name: 'mdnBuiltins_',
      factory: function () {
        return this.bcd_.javascript.builtins;
      },
    },
  ],
  ...
  methods: [
    {
      ...
      code: async function () {
        ...
        const confluenceReleases = await this.fetchJson_(
          this.confluenceReleaseUrl
        );
        ...
        confluenceReleases.forEach((confluenceRelease, columnIndex) => {
          ...
          const [browser, version] = this.getMdnBrowserRelease_(
            confluenceRelease
          );
          if (!browser || !version) {
            return;
          ...
          let release = releases.find(
            (r) => r.browser === browser && r.version === version
          );
          if (!release) {
            release = { browser, version, confluence: [] };
            releases.push(release);
          }
          release.confluence.push(
            Object.assign({ columnIndex }, confluenceRelease)
          );
        });
        ...
        for (const iface of ifaceKeys) {
          ...
          const apiKeys = Object.keys(ifaces[iface]).filter(
            (api) =>
              !(
                /[^a-z]/i.test(api) ||
                !(
          ...
          for (const api of apiKeys) {
            const confluenceRow = confluenceData.find(
              (r) => r.id === `${iface}#${api}`
            );
    ...
    },
    {
      ...
      code: function (release) {
        ...
        const targetVersion =
          browser === 'safari_ios' ? osVersion : browserVersion;
        ...
        return new Promise((resolve, reject) => {
          http.get(url, (res) => {
            if (!(res.statusCode >= 200 && res.statusCode <= 299)) {
              reject(
                `Bad response ${res.statusCode} ${res.statusMessage} for ${url}`
              );
            }
            const chunks = [];
